import pygame as pg
import math
from settings import *
from pygame import Vector2
from random import uniform, randint


class Wall(pg.sprite.Sprite):
    def __init__(self, game, x, y):
        self.groups = game.all_sprites, game.walls
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        # self.image = pg.Surface((TILESIZE, TILESIZE))
        # self.image.fill(GREEN)
        self.image = game.tile_image
        self.rect = self.image.get_rect()
        self.x, self.y = x, y
        self.rect.x, self.rect.y = x * TILESIZE, y * TILESIZE


class Mob(pg.sprite.Sprite):
    def __init__(
        self, game, groups, position, max_speed, acceleration, max_health, color
    ):
        self.groups = groups
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        # self.image = pg.Surface((TILESIZE, TILESIZE))
        # self.image.fill(color)
        self.image = game.virus_image
        self.rect = self.image.get_rect()
        self.weapon_name = "MACHINEGUN"
        self.last_shot_time = 0

        self.max_speed = max_speed + uniform(-max_speed * 0.5, max_speed * 0.5)
        self.acceleration = acceleration
        self.position = position
        self.rect.topleft = position
        self.desired_velocity = Vector2(0, 0)
        self.velocity = Vector2(0, 0)
        self.avoidance = Vector2(0, 0)
        self.max_health = max_health
        self.health = max_health

    def update(self):
        pass

    def move(self):
        if self.desired_velocity.magnitude() > 0:
            self.desired_velocity = self.desired_velocity.normalize()

        self.velocity -= self.velocity * DRAG * self.game.dt
        self.velocity += (
            (self.desired_velocity + self.avoidance) * self.acceleration * self.game.dt
        )
        if self.velocity.magnitude() > self.max_speed:
            self.velocity.scale_to_length(self.max_speed)

        self.position += self.velocity * self.game.dt

        self.rect.x = self.position.x
        self.collide_with_walls("x")
        self.rect.y = self.position.y
        self.collide_with_walls("y")

    def collide_with_walls(self, dir):
        hits = pg.sprite.spritecollide(self, self.game.walls, False)
        if len(hits) == 0:
            return

        if dir == "x":
            if self.velocity.x > 0:
                self.position.x = hits[0].rect.left - self.rect.width
            if self.velocity.x < 0:
                self.position.x = hits[0].rect.right
            self.velocity.x = 0
            self.rect.x = self.position.x

        if dir == "y":
            if self.velocity.y > 0:
                self.position.y = hits[0].rect.top - self.rect.height
            if self.velocity.y < 0:
                self.position.y = hits[0].rect.bottom
            self.velocity.y = 0
            self.rect.y = self.position.y

    def receive_damage(self, damage):
        self.health -= damage
        if self.health <= 0:
            self.health = 0
            self.game.explosion_fx.play()
            self.kill()

    def draw_health(self):
        health = self.health / self.max_health

        bar_width = int(health * self.rect.width)
        health_bar = pg.Rect(self.position.x, self.position.y - 7, bar_width, 5)

        pg.draw.rect(self.game.display, GREEN, health_bar)

    def avoid_mobs(self):
        towards_mobs = Vector2(0, 0)
        for mob in self.game.mobs:
            if mob != self:
                towards_mob = mob.position - self.position
                if 0 < towards_mob.magnitude() < AVOID_RADIUS:
                    towards_mobs += towards_mob / towards_mob.magnitude()

        self.avoidance = self.avoidance.lerp(-towards_mobs, self.game.dt)

    def shoot_at(self, x, y, target_group):
        weapon = WEAPONS[self.weapon_name]

        time_since_last_shot = pg.time.get_ticks() - self.last_shot_time
        if time_since_last_shot < weapon["FIRING_RATE"]:
            return

        bullet_velocity = Vector2(x, y) - self.position
        if bullet_velocity.magnitude() > 0:
            bullet_velocity = bullet_velocity.normalize()

        for _ in range(weapon["AMMO_PER_SHOT"]):
            Bullet(
                self.game,
                Vector2(self.rect.centerx, self.rect.centery),
                bullet_velocity,
                weapon["SPREAD"],
                weapon["TTL"],
                weapon["SPEED"],
                weapon["DAMAGE"],
                weapon["COLOR"],
                weapon["SIZE"],
                target_group,
            )

        self.last_shot_time = pg.time.get_ticks()


class Player(Mob):
    def __init__(self, game, position, max_speed, acceleration, max_health, color):
        super().__init__(
            game,
            (game.all_sprites, game.players),
            position,
            max_speed,
            acceleration,
            max_health,
            color,
        )
        self.image = game.player_image

    def update(self):
        self.handle_input()
        self.move()

    def handle_input(self):
        vx, vy = 0, 0
        key = pg.key.get_pressed()

        if key[pg.K_LEFT] or key[pg.K_a]:
            vx = -1
        if key[pg.K_RIGHT] or key[pg.K_d]:
            vx = 1
        if key[pg.K_UP] or key[pg.K_w]:
            vy = -1
        if key[pg.K_DOWN] or key[pg.K_s]:
            vy = 1

        self.desired_velocity = Vector2(vx, vy)

        mouse = pg.mouse.get_pressed()
        if mouse[0]:
            x, y = pg.mouse.get_pos()
            self.shoot_at(x, y, self.game.mobs)

    def die(self):
        if self.health == 0:
            print("Game Over!")


class Bee(Mob):
    def __init__(
        self,
        game,
        position,
        max_speed,
        acceleration,
        max_health,
        damage,
        color,
        groups=(),
    ):
        super().__init__(
            game,
            (game.all_sprites, game.mobs) + groups,
            position,
            max_speed,
            acceleration,
            max_health,
            color,
        )
        self.damage = damage

    def update(self):
        towards_player = self.game.player.position - self.position
        if towards_player.magnitude() < BEE_VISION_RADIUS:
            self.desired_velocity = towards_player
        else:
            self.desired_velocity = Vector2(uniform(-1, 1), uniform(-1, 1))
        self.avoid_mobs()
        self.move()

        if pg.sprite.collide_rect(self, self.game.player):
            self.game.player.receive_damage(self.damage * self.game.dt)


class BeeNest(Mob):
    def __init__(
        self, game, position, max_health, spawn_frequency, max_population, color
    ):
        super().__init__(
            game,
            (game.all_sprites, game.nests, game.mobs),
            position,
            0,
            0,
            max_health,
            color,
        )
        self.image = game.nest_image
        self.spawn_frequency = spawn_frequency
        self.last_spawn_time = 0
        self.max_population = max_population
        self.population = pg.sprite.Group()

    def update(self):
        time_since_last_spawn = pg.time.get_ticks() - self.last_spawn_time
        time_has_passed = time_since_last_spawn >= self.spawn_frequency
        is_room_left = len(self.population) < self.max_population

        if time_has_passed and is_room_left:
            max_spawneable = self.max_population - len(self.population)
            to_spawn = randint(1, max_spawneable)
            for _ in range(to_spawn):
                Bee(
                    self.game,
                    Vector2(self.position.x, self.position.y)
                    + Vector2(
                        uniform(-TILESIZE, TILESIZE), uniform(-TILESIZE, TILESIZE)
                    ),
                    BEE_MAX_SPEED,
                    BEE_ACCELERATION,
                    BEE_HEALTH,
                    BEE_HIT_DAMAGE,
                    ORANGE,
                    (self.population,),
                )
            self.last_spawn_time = pg.time.get_ticks()


class Tower(Mob):
    def __init__(self, game, position):
        max_health = MOBS["TOWER"]["HEALTH"]
        color = MOBS["TOWER"]["COLOR"]
        super().__init__(
            game, (game.all_sprites, game.mobs), position, 0, 0, max_health, color
        )
        self.weapon_name = MOBS["TOWER"]["WEAPON_NAME"]
        self.image = game.tower_image

    def update(self):
        target = self.game.player.position
        towards_player = self.game.player.position - self.position

        if 0 < towards_player.magnitude() < 200:
            self.shoot_at(target.x, target.y, self.game.players)


class Bullet(pg.sprite.Sprite):
    def __init__(
        self,
        game,
        position,
        velocity,
        spread,
        ttl,
        speed,
        damage,
        color,
        size,
        target_group,
    ):
        self.groups = game.all_sprites, game.bullets
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        # self.image = pg.Surface((size, size))
        # self.image.fill(color)
        self.image = game.bullet_image
        self.rect = self.image.get_rect()

        self.position = position
        self.rect.center = self.position
        self.ttl = ttl
        self.spawn_time = pg.time.get_ticks()
        self.speed = uniform(speed * 0.9, speed * 1.1)
        self.damage = damage
        self.velocity = velocity + Vector2(
            uniform(-spread, spread), uniform(-spread, spread)
        )
        self.velocity = self.velocity.normalize()
        self.target_group = target_group

    def update(self):
        self.position += self.velocity * self.speed * self.game.dt
        self.rect.center = self.position

        life_time = pg.time.get_ticks() - self.spawn_time
        if life_time > self.ttl:
            self.kill()

        if pg.sprite.spritecollide(self, self.game.walls, False):
            self.kill()

        hits = pg.sprite.spritecollide(self, self.target_group, False)
        if len(hits) > 0:
            hits[0].receive_damage(self.damage)
            self.kill()


class Item(pg.sprite.Sprite):
    def __init__(self, game, position, kind):
        self.groups = game.all_sprites, game.items
        pg.sprite.Sprite.__init__(self, self.groups)
        self.game = game
        self.image = pg.Surface((TILESIZE, TILESIZE))
        self.image.fill(ITEMS[kind]["COLOR"])
        self.rect = self.image.get_rect()
        self.position = position
        self.rect.topleft = position
        self.kind = kind

    def update(self):
        self.rect.top = (
            self.position.y
            + math.sin(pg.time.get_ticks() * ITEM_HOVER_SPEED) * TILESIZE // 2
            + TILESIZE // 2
        )

        if pg.sprite.collide_rect(self, self.game.player):
            self.picked_by(self.game.player)

    def picked_by(self, picker):
        pass


class HealthPack(Item):
    def __init__(self, game, position):
        super().__init__(game, position, "HEALTHPACK")
        self.image = game.heal_image

    def picked_by(self, picker):
        heal = ITEMS["HEALTHPACK"]["HEAL"]
        if picker.health < picker.max_health:
            picker.health = min(picker.health + heal, picker.max_health)
            self.game.heal_fx.play()
            self.kill()


class SpeedUp(Item):
    def __init__(self, game, position):
        super().__init__(game, position, "SPEEDUP")
        self.picker = None
        self.image = game.speed_image

    def picked_by(self, picker):
        self.picker = picker
        speed_up = ITEMS[self.kind]["SPEED"]
        picker.max_speed += speed_up
        ttl = ITEMS[self.kind]["TTL"]
        self.stop_working_at = pg.time.get_ticks() + ttl
        self.rect.x = -1000000
        self.game.speed_fx.play()

    def update(self):
        if self.picker == None:
            super().update()
            return

        now = pg.time.get_ticks()
        if now > self.stop_working_at:
            speed_up = ITEMS[self.kind]["SPEED"]
            self.picker.max_speed -= speed_up
            self.kill()
